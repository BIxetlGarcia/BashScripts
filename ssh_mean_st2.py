...
# In[3]:
mfiles=sorted(glob.glob('/ccc/scratch/cont003/gen0727/garciagi/DETREND/GOM025.GSL301*.nc'))
...
ncols=lats.shape[1]            
print(len(mfiles))              
meanS= NetCDFFile('/ccc/scratch/cont003/gen0727/garciagi/SSTmean.nc', 'w', format='NETCDF3_64BIT')
...
meansst  = meanS.createVariable('meansst', 'f4', ('time_counter', 'y','x'))
varsst  = meanS.createVariable('varsst', 'f4', ('time_counter', 'y','x'))
varsst2  = meanS.createVariable('varsst2', 'f4', ('time_counter', 'y','x'))
...
meansst[:,:,:]=stm.data/50.
varsst[:,:,:] =stdm.data/50.
varsst2[:,:,:] =(stdm.data/50.)-((stm.data/50.)*(stm.data/50.))
varssti[:,:,:]=sti
filread[:]=fils
meanS.close()
